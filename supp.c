...
unsigned long long readval(int be,void *src,int size)
...
void *setval(int be,void *dest,int size,unsigned long long val)
...
{
  if ((bfsize&7)==0 && offset+size<=bfsize) {
    unsigned long long mask = MAKEMASK(size);
    unsigned long long val = readval(be,p,bfsize>>3);
