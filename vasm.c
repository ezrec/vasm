...
char *inname=0,*outname=0,*listname=0;
int secname_attr=0;
int unnamed_sections=0;
int ignore_multinc=0;
int nocase=0;
int no_symbols=0;
int pic_check=0;
int final_pass,produce_listing,listformfeed=1,listlinesperpage=40,listnosyms;
...
#define MNEMOHTABSIZE 0x200
#endif
hashtable *mnemohash;

#ifndef SYMHTABSIZE
#define SYMHTABSIZE 0x4000
#endif
...
static void statistics(void)
{
  ...
  printf("\n");
  for(sec=first_section;sec;sec=sec->next)
    printf("%s(%s%d):\t%12llu byte%c\n",sec->name,sec->attr,sec->align,
           (unsigned long long)(sec->pc-sec->org),
           sec->pc-sec->org==1?' ':'s');
}
