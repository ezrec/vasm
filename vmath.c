...
unsigned char *int96_assign64(unsigned char *x,long long val)
{
  int i;
...
unsigned char *int96_add32(unsigned char *x,long val)
{
  int96 y;
...
unsigned char *int96_add64(unsigned char *x,long long val)
{
  int96 y;
...
unsigned char *int96_mulu(unsigned char *x,unsigned char *y)
{
  ...
  for (i=0; i<12; i++) {
    for (j=0; j<(12-i); j++) {
      m = (unsigned short)x[i] * (unsigned short)y[j];
      p[i+j] = m & 0xff;
      p[i+j+1] = (m >> 8) & 0xff;
      int96_add(sum,p);
      p[i+j] = p[i+j+1] = 0;
